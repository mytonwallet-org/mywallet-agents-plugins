...
"use strict";
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   i: () => (/* binding */ createLockedJsonFileStore)
/* harmony export */ });
...
async function acquireFileLock(filePath) {
  ...
  await (0,node_fs_promises__WEBPACK_IMPORTED_MODULE_1__.mkdir)(dirnamePath(filePath), {
    recursive: true
  });
  for (let attempt = 1; attempt <= LOCK_MAX_ATTEMPTS; attempt += 1) {
    try {
      await (0,node_fs_promises__WEBPACK_IMPORTED_MODULE_1__.mkdir)(lockDirPath);
      await (0,node_fs_promises__WEBPACK_IMPORTED_MODULE_1__.writeFile)(joinFilePath(lockDirPath, LOCK_METADATA_FILE_NAME), lockMetadata, 'utf8');
      return async () => {
        try {
          await (0,node_fs_promises__WEBPACK_IMPORTED_MODULE_1__.rm)(lockDirPath, {
            recursive: true,
            force: true
          });
        } catch (err) {
...
async function isStaleLock(lockDirPath) {
  try {
    const lockStat = await (0,node_fs_promises__WEBPACK_IMPORTED_MODULE_1__.stat)(lockDirPath);
...
async function readJsonFile(filePath, reviveValue) {
  try {
    const content = await (0,node_fs_promises__WEBPACK_IMPORTED_MODULE_1__.readFile)(filePath, 'utf8');
...
async function writeJsonFile(filePath, data, replaceValue) {
  ...
  const tempFilePath = `${filePath}.${process.pid}.${(0,node_crypto__WEBPACK_IMPORTED_MODULE_0__.randomUUID)()}.tmp`;
  const content = JSON.stringify(data, replaceValue);
  await (0,node_fs_promises__WEBPACK_IMPORTED_MODULE_1__.writeFile)(tempFilePath, content, 'utf8');
  await (0,node_fs_promises__WEBPACK_IMPORTED_MODULE_1__.rename)(tempFilePath, filePath);
}
