...
This directory is the repo-owned, user-facing Claude Code offering for My Wallet.
...
`My Wallet for Claude Code` is a local Claude Code integration that runs the packaged `mywallet-mcp` server on your machine and exposes the canonical `mywallet_*` wallet tools.
It is built from the shared plugin root in `headless/platforms/shared-plugin/` so Claude Code stays aligned with the same runtime semantics, launcher contract, and approval-aware wallet behavior as the other supported hosts.

## What you get

After installing the staged offering into Claude Code, you get local access to the canonical My Wallet tool surface, including flows such as:
...
- a Claude Code marketplace/store publication that is already live
- host-native approval bypasses for transfers or signing

This is a **direct-install-first** Claude Code offering built from the repo and staged as a local installable artifact.
...
## Build the installable Claude Code artifact

From the repo root:

```bash
npm run build:headless:claude-code
```
...
- `mywallet-claude-code-v<version>.zip` — the end-user download/install artifact
...
The staged root is a Claude Code-facing direct-install package built from:

- the shared plugin manifest and Claude Code wrapper manifest
- the shared `.mcp.json`
- the shared launcher
...
Use the staged directory or the staged zip as the install source for your local Claude Code plugin/import flow.
...
```text
headless/platforms/claude-code/.dist/releases/mywallet-claude-code-v<version>/mywallet-claude-code-v<version>.zip
```
...
The exact UI wording in Claude Code may change over time, but the install target should be the staged offering root or its zip archive — **not** the raw `shared-plugin/` source tree.
...
```bash
npm run test:headless:claude-code:isolation -- \
  --bundle-archive ./headless/platforms/claude-code/.dist/releases/mywallet-claude-code-v<version>/mywallet-claude-code-v<version>.zip \
  --output ./headless/platforms/claude-code/.dist/isolation-report.json
```

The harness unpacks the archive in a temp directory, verifies the expected Claude Code-facing files are present, launches the bundled MCP server through the staged launcher, and proves `initialize`, `tools/list`, and `mywallet_status` against the release-prepared artifact.
...
### Claude Code import/install does not like the raw repo files

Use the staged `.dist/mywallet-claude-code/` directory or `.dist/mywallet-claude-code.zip` archive.
Do not point Claude Code at `headless/platforms/shared-plugin/` directly if you want the tested release shape.
...
If that passes but Claude Code still misbehaves, treat it as a host-side integration issue and keep release claims conservative.

### I need a lower-level fallback path

The shared plugin root still contains the thin Claude Code wrapper manifest at:

- `headless/platforms/shared-plugin/.claude-plugin/plugin.json`

That is the architecture-level substrate.
The supported user-facing release surface in this repo is this `headless/platforms/claude-code/` offering directory and its staged `.dist/` output.
