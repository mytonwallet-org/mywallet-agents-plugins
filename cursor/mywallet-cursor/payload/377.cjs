...
var capacitorStorage = __webpack_require__(84773);
// EXTERNAL MODULE: ./src/api/storages/extension.ts
var extension = __webpack_require__(29419);
// EXTERNAL MODULE: ./src/api/storages/idb.ts
var idb = __webpack_require__(78839);
...
function getStorageGlobals() {
  const g = globalThis;
  if (!g[STORAGE_GLOBALS_KEY]) {
    g[STORAGE_GLOBALS_KEY] = {};
  }
  return g[STORAGE_GLOBALS_KEY];
}
...
function createNodeFileStorage(storageConfig) {
  ...
  // Literal relative require so webpack resolves and bundles `./nodeFile` statically. Going
  // through a wrapper would route us to webpack's empty-context stub at runtime, which throws
  // `Cannot find module './nodeFile'` even though the module is in the chunk graph. Browser
  // builds never reach this branch because their default storage selection short-circuits
  // before `nodeFile` is requested.
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const nodeFileModule = __webpack_require__(54330);
  return nodeFileModule.createNodeFileStorage(storageConfig);
}
function loadBundledNodeFileStorageModule() {
  var _process$mainModule;
  const payloadDir = {"APP_ENV":"test","IS_HEADLESS":"1"}.MTW_HEADLESS_BUNDLE_PAYLOAD_DIR;
  if (!payloadDir) {
    return undefined;
  }
...
function getStorageContext() {
  ...
  try {
    ...
    globals.storageContext = new AsyncLocalStorage();
...
function isBrowserSingleRuntimeStorageAllowed() {
  return typeof window !== 'undefined' || typeof document !== 'undefined' || typeof importScripts === 'function' || typeof WorkerGlobalScope !== 'undefined' || Boolean({"APP_ENV":"test","IS_HEADLESS":"1"}.MTW_HEADLESS_BUNDLE_PAYLOAD_DIR);
}
function getOptionalRequire() {
  ...
  if (true) {
    return __webpack_require__(95023);
  }
...
// EXTERNAL MODULE: ./src/api/storages/nodeFile.ts
var nodeFile = __webpack_require__(54330);
;// ./headless/storage.ts
/* unused harmony import specifier */ var storage_withStorage;
...
function runWithHeadlessStorage(runtimeStorage, fn) {
  return storage_withStorage(runtimeStorage, fn);
}
function resolveSrcApiInitArgs(apiInitArgs) {
  // Pass storage config through to `init.ts` so it sets up the AsyncLocalStorage scope with the
  // correct nodeFile storage instead of falling back to the browser-only IndexedDB default.
  return apiInitArgs;
}
...
function resolveHeadlessStoragePath() {
  ...
  const configuredPath = (_process$env$STORAGE_ = {"APP_ENV":"test","IS_HEADLESS":"1"}.STORAGE_FILENAME) === null || _process$env$STORAGE_ === void 0 ? void 0 : _process$env$STORAGE_.trim();
...
const EVM_CAPABILITIY_MAP = {
  ...
  fungibleTokenBalances: {
    status: 'supported',
    notes: 'ERC-20 token balances ship today; downstream value enrichment can still ' + 'degrade when token metadata is unresolved.'
  },
  collectibles: {
    status: 'partial',
  ...
  transfers: {
    status: 'supported',
    notes: 'Native and ERC-20 transfers ship today through `prepareTransfer` and ' + '`submitTransfer`; omitted `tokenAddress` keeps native-transfer semantics.'
  },
...
const HEADLESS_CAPABILITY_MATRIX = {
  ...
  ethereum: EVM_CAPABILITIY_MAP,
  base: EVM_CAPABILITIY_MAP,
  arbitrum: EVM_CAPABILITIY_MAP,
  bnb: EVM_CAPABILITIY_MAP,
  hyperliquid: {
    ...EVM_CAPABILITIY_MAP,
    collectibles: {
      status: 'unsupported'
...
async function executeHeadlessCli(options) {
  ...
  try {
    const {
      ...
      cwd: REPO_ROOT,
      env: {
        ...{"APP_ENV":"test","IS_HEADLESS":"1"},
        APP_ENV: {"APP_ENV":"test","IS_HEADLESS":"1"}.APP_ENV ?? 'test'
      },
...
/* unused harmony import specifier */ var IS_HEADLESS;
/* unused harmony import specifier */ var IS_CAPACITOR;
/* unused harmony import specifier */ var IS_EXTENSION;
...
const HEADLESS_ORIGIN = 'https://agents.mywallet.io';
let environment;
function getAppOrigin(args) {
  if (IS_HEADLESS) {
    return HEADLESS_ORIGIN;
  }
  if (args.isElectron) {
    return ELECTRON_ORIGIN;
  }
  if (IS_CAPACITOR || IS_EXTENSION) {
    var _self;
...
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   createNodeFileStorage: () => (/* binding */ createNodeFileStorage),
/* harmony export */   resolveDefaultNodeFileStoragePath: () => (/* binding */ resolveDefaultNodeFileStoragePath)
/* harmony export */ });
...
/* harmony import */ var _util_lockedJsonFile__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(34923);
...
function createNodeFileStorage(config) {
  ...
  const fileStore = (0,_util_lockedJsonFile__WEBPACK_IMPORTED_MODULE_1__/* .createLockedJsonFileStore */ .i)({
    filePath,
...
function resolveDefaultNodeFileStoragePath() {
  ...
  return joinFilePath((0,node_os__WEBPACK_IMPORTED_MODULE_0__.homedir)(), `.${HEADLESS_STORAGE_DIR_NAME}`, profile, HEADLESS_STORAGE_FILE_NAME);
}
...
function expandNodeFileStoragePath(filePath) {
  return filePath.replace(/\$\{HOME\}/g, (0,node_os__WEBPACK_IMPORTED_MODULE_0__.homedir)());
}
...
const TEST_MNEMONIC = (_process$env$TEST_MNE = {"APP_ENV":"test","IS_HEADLESS":"1"}.TEST_MNEMONIC) === null || _process$env$TEST_MNE === void 0 ? void 0 : _process$env$TEST_MNE.trim();
const TEST_PASSWORD = {"APP_ENV":"test","IS_HEADLESS":"1"}.TEST_PASSWORD || 'test';
const IS_PRODUCTION = APP_ENV === 'production';
const IS_TEST = APP_ENV === 'test';
const IS_PERF = APP_ENV === 'perf';
const IS_EXTENSION = {"APP_ENV":"test","IS_HEADLESS":"1"}.IS_EXTENSION === '1';
const IS_FIREFOX_EXTENSION = {"APP_ENV":"test","IS_HEADLESS":"1"}.IS_FIREFOX_EXTENSION === '1';
const IS_OPERA_EXTENSION = {"APP_ENV":"test","IS_HEADLESS":"1"}.IS_OPERA_EXTENSION === '1';
const IS_PACKAGED_ELECTRON = {"APP_ENV":"test","IS_HEADLESS":"1"}.IS_PACKAGED_ELECTRON === '1';
const IS_ANDROID = {"APP_ENV":"test","IS_HEADLESS":"1"}.PLATFORM_ENV === 'Android';
const IS_CAPACITOR = {"APP_ENV":"test","IS_HEADLESS":"1"}.IS_CAPACITOR === '1';
const IS_ANDROID_DIRECT = {"APP_ENV":"test","IS_HEADLESS":"1"}.IS_ANDROID_DIRECT === '1';
const IS_AIR_APP = {"APP_ENV":"test","IS_HEADLESS":"1"}.IS_AIR_APP === '1';
const IS_TELEGRAM_APP = {"APP_ENV":"test","IS_HEADLESS":"1"}.IS_TELEGRAM_APP === '1';
const IS_EXPLORER = {"APP_ENV":"test","IS_HEADLESS":"1"}.IS_EXPLORER === '1';
const IS_HEADLESS = {"APP_ENV":"test","IS_HEADLESS":"1"}.IS_HEADLESS === '1';
...
const APP_INSTALL_URL = (/* unused pure expression or super */ null && (IS_GRAM_WALLET ? 'https://get.gramwallet.io/' : 'https://get.mytonwallet.io/'));
const APP_REPO_URL = 'https://github.com/mytonwallet-org/mytonwallet';
const SELF_UNIVERSAL_HOST_URL = 'https://my.tt';
const APP_WEBSITE_URL = (/* unused pure expression or super */ null && (IS_GRAM_WALLET ? 'https://gramwallet.io' : 'https://mytonwallet.io'));
const APP_ICON_URL = (/* unused pure expression or super */ null && (IS_GRAM_WALLET ? 'https://gramwallet.io/icon-512x512.png' : 'https://mytonwallet.io/icon-512x512.png'));

// GitHub workflow uses an empty string as the default value if it's not in repository variables, so we cannot define a default value here
const BASE_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.BASE_URL || PRODUCTION_URL;
const BOT_USERNAME = {"APP_ENV":"test","IS_HEADLESS":"1"}.BOT_USERNAME || 'MyTonWalletBot';
const SWAP_FEE_ADDRESS = {"APP_ENV":"test","IS_HEADLESS":"1"}.SWAP_FEE_ADDRESS || 'UQDUkQbpTVIgt7v66-JTFR-3-eXRFz_4V66F-Ufn6vOg0GOp';
const DIESEL_ADDRESS = {"APP_ENV":"test","IS_HEADLESS":"1"}.DIESEL_ADDRESS || 'UQC9lQOaEHC6YASiJJ2NrKEOlITMMQmc8j0_iZEHy-4sl3tG';
...
const TONCENTER_MAINNET_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.TONCENTER_MAINNET_URL || 'https://toncenter.mytonwallet.org';
const TONCENTER_MAINNET_KEY = {"APP_ENV":"test","IS_HEADLESS":"1"}.TONCENTER_MAINNET_KEY;
const ELECTRON_TONCENTER_MAINNET_KEY = {"APP_ENV":"test","IS_HEADLESS":"1"}.ELECTRON_TONCENTER_MAINNET_KEY;
const TONAPIIO_MAINNET_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.TONAPIIO_MAINNET_URL || 'https://tonapiio.mytonwallet.org';
const TONCENTER_TESTNET_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.TONCENTER_TESTNET_URL || 'https://toncenter-testnet.mytonwallet.org';
const TONCENTER_TESTNET_KEY = {"APP_ENV":"test","IS_HEADLESS":"1"}.TONCENTER_TESTNET_KEY;
const ELECTRON_TONCENTER_TESTNET_KEY = {"APP_ENV":"test","IS_HEADLESS":"1"}.ELECTRON_TONCENTER_TESTNET_KEY;
const TONAPIIO_TESTNET_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.TONAPIIO_TESTNET_URL || 'https://tonapiio-testnet.mytonwallet.org';
const BRILLIANT_API_BASE_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.BRILLIANT_API_BASE_URL || 'https://api.mytonwallet.org';
const PROXY_API_BASE_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.PROXY_API_BASE_URL || 'https://api.mytonwallet.org/proxy';
...
const WALLET_CONNECT_PROJECT_ID = {"APP_ENV":"test","IS_HEADLESS":"1"}.WALLET_CONNECT_PROJECT_ID || '';
const TRON_MAINNET_API_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.TRON_MAINNET_API_URL || 'https://tronapi.mytonwallet.org';
const TRON_TESTNET_API_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.TRON_TESTNET_API_URL || 'https://api.shasta.trongrid.io';
const SOLANA_MAINNET_RPC_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.SOLANA_MAINNET_RPC_URL || 'https://solanaapi.mytonwallet.org';
const SOLANA_MAINNET_API_KEY = {"APP_ENV":"test","IS_HEADLESS":"1"}.SOLANA_MAINNET_API_KEY;
const SOLANA_TESTNET_RPC_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.SOLANA_TESTNET_RPC_URL || 'https://solanaapi-devnet.mytonwallet.org';
const SOLANA_TESTNET_API_KEY = {"APP_ENV":"test","IS_HEADLESS":"1"}.SOLANA_TESTNET_API_KEY;
const SOLANA_TESTNET_API_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.SOLANA_TESTNET_API_URL || 'https://solanaapi-devnet.mytonwallet.org';
const SOLANA_MAINNET_API_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.SOLANA_MAINNET_API_URL || 'https://solanaapi.mytonwallet.org';
const SOLANA_GASLESS_PAYER_ADDRESS = {"APP_ENV":"test","IS_HEADLESS":"1"}.SOLANA_GASLESS_PAYER_ADDRESS || 'BkVfRKjZnnYCcRBgXBsfaWFZFidBL9drm5MZwNqoNGCu';
const EVM_MAINNET_RPC_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.EVM_MAINNET_RPC_URL || 'https://evmapi.mytonwallet.org';
const EVM_TESTNET_RPC_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.EVM_TESTNET_RPC_URL || 'https://evmapi-testnet.mytonwallet.org';
...
const MULTISEND_DAPP_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.MULTISEND_DAPP_URL || 'https://multisend.mytonwallet.io/';
const PORTFOLIO_DAPP_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.PORTFOLIO_DAPP_URL || 'https://portfolio.mytonwallet.io/';
const PORTFOLIO_API_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.PORTFOLIO_API_URL || 'https://api-portfolio.mytonwallet.io/api';
const AGENT_API_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.AGENT_API_URL || 'https://agent.mytonwallet.org/api';
const NFT_MARKETPLACE_URL = 'https://opensea.io/';
const NFT_MARKETPLACE_TITLE = NFT_MARKETPLACE_TITLES.opensea;
const TON_NFT_MARKETPLACE_URL = 'https://getgems.io/';
const TON_NFT_MARKETPLACE_TITLE = NFT_MARKETPLACE_TITLES.getgems;
...
const PROXY_HOSTS = {"APP_ENV":"test","IS_HEADLESS":"1"}.PROXY_HOSTS;
...
const LANG_CACHE_NAME = 'mtw-lang-296';
const LANG_LIST = [{
  langCode: 'en',
  name: 'English',
  nativeName: 'English',
...
const STAKING_POOLS = {"APP_ENV":"test","IS_HEADLESS":"1"}.STAKING_POOLS ? {"APP_ENV":"test","IS_HEADLESS":"1"}.STAKING_POOLS.split(' ') : [];
const LIQUID_POOL = {"APP_ENV":"test","IS_HEADLESS":"1"}.LIQUID_POOL || 'EQD2_4d91M4TVbEBVyBF8J1UwpMJc361LKVCz6bBlffMW05o';
const LIQUID_JETTON = {"APP_ENV":"test","IS_HEADLESS":"1"}.LIQUID_JETTON || 'EQCqC6EhRJ_tpWngKxL6dV0k6DSnRUrs9GSVkLbfdCqsj6TE';
...
const HYPERLIQUID_USDC_MAINNET = {
  name: 'USD Coin',
  symbol: 'USDC',
  decimals: 6,
  chain: 'hyperliquid',
...
const PRIORITY_TOKENS = [ETH, SOLANA, TRX, BNB, TONCOIN, HYPERLIQUID];
...
const GIVEAWAY_CHECKIN_URL = {"APP_ENV":"test","IS_HEADLESS":"1"}.GIVEAWAY_CHECKIN_URL || 'https://giveaway.mytonwallet.io';
...
/* harmony import */ var _config__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(28732);
/* harmony import */ var _decimals__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(49889);
...
/* harmony import */ var _config__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(28732);
/* harmony import */ var _lib_big_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(33632);
...
/* harmony import */ var _config__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(28732);
/* harmony import */ var _assert__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(15247);
...
var airAppCallWindow = __webpack_require__(95525);
// EXTERNAL MODULE: ./src/util/bigint.ts
var bigint = __webpack_require__(41666);
...
var logs = __webpack_require__(29414);
...
/* unused harmony import specifier */ var WINDOW_PROVIDER_PORT;
/* unused harmony import specifier */ var WINDOW_PROVIDER_CHANNEL;
/* unused harmony import specifier */ var connector_createReverseExtensionConnector;
...
function initWindowConnector() {
  ...
  if ({"APP_ENV":"test","IS_HEADLESS":"1"}.IS_EXTENSION) {
    connector = connector_createReverseExtensionConnector(WINDOW_PROVIDER_PORT);
